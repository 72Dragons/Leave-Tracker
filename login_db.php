...
{
	global $con;
	$query="SELECT clearanceLevel from staffinfo  where memberID=?";
	$result=$con->prepare($query);
	$result->bind_param('i',$id);
	$result->execute();
...
$to_date=date_format($T_date,"d F Y");
						    ...
						    $mail->setFrom('siddhant@72dragons.com',$value[0]['user_name']);
						    $mail->addAddress($value['admin']);     // Add a recipient
						  
						    // Content
						    $mail->isHTML(true);                                  // Set email format to HTML
						    $mail->Subject = $value[0]['user_name'].' Has Applied For A Leave';
...
<body style="background: #ad9440; ">
    <div class="mail_body_container" style="border: 1px solid #000000; margin: 2em;">
        <div class="mail_body_header" style="text-align: center; padding: 2em; border-bottom: 1px solid #000000;">
            ...
            <p style="margin-top: 1em;">'.$value[0]['user_name'].' as requested '.$value['type'].' for the following dates : '.$from_date.	' to '.$to_date.'</p>
        </div>
...
// 						    $mail->addAddress($value[0]['user']);     // Add a recipient
						...
						try {
						    ...
						    //Recipients
						    		foreach ($arr as $key => $value) {
						    			if($key==0)
						    			{
						   	$mail->setFrom('siddhant@72dragons.com',$arr[0]['admin']);
						    $mail->addAddress($arr[0][0]['user']);     // Add a recipient						    				
						    			}
						    			elseif($key==1)
						    			{
							   	$mail->setFrom('siddhant@72dragons.com',$arr[1]['admin']);
						    $mail->addAddress($arr[1][0]['user']);     // Add a recipient						    					
						    			}
						    			elseif($key==2)
						    			{
							   	$mail->setFrom('siddhant@72dragons.com',$arr[2]['admin']);
						    $mail->addAddress($arr[2][0]['user']);     // Add a recipient						    					
						    			}
						    ...
						    // Content
						    $mail->isHTML(true);                                  // Set email format to HTML
						    $mail->Subject = 'Leave '.$arr['status'];
						    $mail->Body   ="Your leave has been ".$arr['status'];
						    //return false;
						    //echo 'Message has been sent';
